...
use crate::components::data_grid::{ColumnInfo, QueryResult};
...
pub fn validate_cql_identifier(name: &str) -> Result<()> {
    ...
    if !first.is_ascii_alphabetic() && first != '_' {
        return Err(MagdaError::validation(format!(
            "Invalid CQL identifier '{}': must start with a letter or underscore",
            name
        )));
    }
    if !name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        return Err(MagdaError::validation(format!(
            "Invalid CQL identifier '{}': only letters, digits, and underscores allowed",
            name
        )));
    }
    Ok(())
...
pub struct CassandraSession {
    inner: Arc<
        cdrs_tokio::cluster::session::Session<
        ...
        match res_result_body {
            cdrs_tokio::frame::message_result::ResResultBody::Rows(rows_result) => {
                ...
                for (i, row) in rows_result.rows_content.iter().enumerate() {
                    if let Some(keyspace_bytes) = row.first() {
    ...
    if let ResponseBody::Result(res_result_body) = result.response_body()? {
        ...
        match res_result_body {
            cdrs_tokio::frame::message_result::ResResultBody::Rows(rows_result) => {
                ...
                for (i, row) in rows_result.rows_content.iter().enumerate() {
                    if let Some(table_bytes) = row.first() {
                        // Extract bytes from CBytes using as_slice() method
                        if let Some(bytes) = table_bytes.as_slice() {
                            match String::from_utf8(bytes.to_vec()) {
                                ...
                                Err(e) => {
                                    tracing::warn!(
                                        "Failed to convert bytes to string for row {}: {:?}",
                                        i,
                                        e
                                    );
                                }
...
pub async fn describe_table(
    ...
    if let ResponseBody::Result(cdrs_tokio::frame::message_result::ResResultBody::Rows(
        rows_result,
...
pub async fn test_connection(session: &CassandraSession) -> Result<()> {
    ...
    if let ResponseBody::Result(res_result_body) = result.response_body()? {
        if let Some(rows_metadata) = res_result_body.as_rows_metadata() {
            tracing::debug!(
                "Connection test: {} columns returned",
                rows_metadata.columns_count
            );
...
fn convert_cassandra_value(bytes: &[u8], column_type: &str) -> Value {
    ...
    match column_type {
        "int" => {
            if bytes.len() == 4 {
                return Value::Number(
                    i32::from_be_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]).into(),
                );
            }
        }
        "bigint" | "counter" => {
            ...
            if bytes.len() >= 4 {
                ...
                if !unscaled_bytes.is_empty() && unscaled_bytes[0] & 0x80 != 0 {
                    for _ in unscaled_bytes.len()..8 {
                        unscaled |=
                            0xFF << (unscaled_bytes.len() * 8 + (7 - unscaled_bytes.len()) * 8);
                    }
                    // Sign-extend
        ...
        "date" => {
            ...
            if bytes.len() == 4 {
                ...
                if let Some(date) =
                    chrono::NaiveDate::from_num_days_from_ce_opt(epoch_days as i32 + 719_163)
                {
